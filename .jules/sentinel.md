## 2026-08-10 - Query Validation Integration at JQL Engine Entrypoint
**Vulnerability:** The central SDK query selection function `select` did not validate incoming query arguments at its entrypoint. Maliciously crafted queries or invalid query parameters supplied by external clients (such as MCP or CLI tools) could bypass structural or operator validation, resulting in unexpected database/index behavior, internal runtime errors, or denial of service through unhandled exceptions in the parsing layer.
**Learning:** Enforcing deep structural and parameters validation at the lowest entrypoint of execution (the core SDK select method) acts as a critical line of defense in depth, ensuring that even if validation is missing or bypassed at outer protocol layers, the query engine remains perfectly secure and robust.
**Prevention:** Integrate a central validation helper (`validateQuery`) directly at the entrypoint of the core query `select` function, throwing a standard, descriptive error that details the validation issues before any query parsing or cache scanning is performed.
