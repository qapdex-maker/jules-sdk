...
## 2026-04-03 - Optimized Unified Diff Parsers to Single-Pass O(N) Algorithm
**Learning:** Splitting large unified git patches by file section (`split(/^diff --git /m)`) and then splitting each file section's lines again (`split('\n')`) generates huge numbers of intermediate substrings, array allocations, and garbage collection churn. Furthermore, calling array methods like `.some()` nested inside these loops compound the overhead. Rewriting the parser into a single-pass line-by-line O(N) scan using simple state accumulators completely eliminates redundant intermediate splits and scans.
**Action:** Avoid double/nested splits on large text payloads like diffs and logs. Standardize on a single-pass line-by-line scanner with lightweight boolean flags/accumulators to process text efficiently with minimal garbage collection.
